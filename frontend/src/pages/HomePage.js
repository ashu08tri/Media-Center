...
const LatestNewsItem = styled.li`
  ...
  &:before { /* Red circle */
    ...
    width: 12px;
    height: 12px;
    background-color: #00712D; /* Theme red color */
    border-radius: 50%;
    z-index: 1;
  }
...
const NewsMeta = styled.div`
  ...
  .category {
    color: #00712D; /* Theme red for category */
    font-weight: 500; 
  }
`;
...
const NewsTitle = styled.h4`
  ...
  a {
    ...
    color: inherit;
    &:hover {
      color: #00712D; /* Theme red on hover */
    }
  }
`;
...
const ExplainsTitle = styled.h2`
  font-size: 28px;
  font-weight: bold;
  color: #00712D; /* Dark red color from image */
  margin: 0;
...
const MainVideoWrapper = styled.div`
  ...
  &:hover .play-icon-overlay {
    background-color: rgba(0,113,45, 0.7); /* More prominent on hover */
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const premium = articlesData.filter(a => (a.category?.toLowerCase() === 'premium' || a.tags?.includes('premium'))).slice(0, 5);
          ...
          <>
            ...
            <ThreeColumnHeroLayout>
              ...
              <HeroColumn>
                ...
                <SideArticleList>
                  {premiumArticles.length > 0 ? premiumArticles.slice(0, 5).map((article, index) => (
                    <PremiumArticleItem key={article._id}>
                      <div className="content">
                        <Link to={`/article/${article.slug}`} className="title">{article.title}</Link>
